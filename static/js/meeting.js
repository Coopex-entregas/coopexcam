...
const socket=io({transports:['websocket','polling'],upgrade:true,reconnection:true,reconnectionAttempts:20,reconnectionDelay:1000});
...
const galleryBtn=document.getElementById('backToGallery');
...
function attachTile({id,name,stream,isLocal=false}){
  ...
  if(!tile){
    ...
    tile.addEventListener('click',()=>selectedVideo?.srcObject===stream?backToGallery():promoteStream(stream,name,!!isLocal));
...
function promoteStream(stream,label,muted=false){
  ...
  if(stageTitle) stageTitle.textContent=label||'Em destaque';
...
function backToGallery(){
  ...
  if(stageTitle) stageTitle.textContent='Galeria';
  if(stageEmpty){
    stageEmpty.style.display='grid';
    stageEmpty.textContent='Selecione um participante ou aguarde quem está com a fala';
  }
...
function renderParticipants(){
  ...
  currentState.participants.forEach(p=>{
    ...
    row.innerHTML=`<div><strong>${p.display_name}</strong><div class="muted small">${p.full_name}</div><div class="participant-meta">${p.online?'<span class="badge green">online</span>':'<span class="badge">offline</span>'}${p.is_eligible?'<span class="badge">apto</span>':''}${p.hand_raised?'<span class="badge">pediu fala</span>':''}${p.mic_blocked?'<span class="badge red">mic bloqueado</span>':''}${p.cam_blocked?'<span class="badge red">cam bloqueada</span>':''}</div></div><div class="row gap wrap">${role==='admin'&&!p.is_admin?`<button class="btn" data-action="toggle_eligible" data-id="${p.id}">Voto</button><button class="btn" data-action="allow_speak" data-id="${p.id}">Liberar</button><button class="btn" data-action="block_mic" data-id="${p.id}">Mic</button><button class="btn" data-action="block_cam" data-id="${p.id}">Cam</button><button class="btn" data-action="spotlight" data-id="${p.id}">Destaque</button><button class="btn danger" data-action="remove" data-id="${p.id}">Remover</button>`:''}</div>`;
...
function renderNotes(){
  if(summaryText) summaryText.value=currentState.room?.summary_text||'';
  if(decisionsText) decisionsText.value=currentState.room?.decisions_text||'';
  if(notesView){
    notesView.innerHTML=`<div class="notes-block"><h3>Resumo</h3><p>${(currentState.room?.summary_text||'').replace(/\n/g,'<br>')||'Sem resumo ainda.'}</p></div><div class="notes-block"><h3>Decisões</h3><p>${(currentState.room?.decisions_text||'').replace(/\n/g,'<br>')||'Sem decisões registradas ainda.'}</p></div>`;
  }
}
...
function setupAdminButtons(){
  ...
  document.getElementById('copyInvite')?.addEventListener('click',async()=>navigator.clipboard.writeText(document.getElementById('inviteUrl').value));
  document.getElementById('copyCamera')?.addEventListener('click',async()=>navigator.clipboard.writeText(document.getElementById('cameraUrl').value));
  document.getElementById('toggleRoom')?.addEventListener('click',async()=>{const r=await api(`/admin/api/room/${roomCode}/toggle_status`); if(r.ok) location.reload();});
  document.getElementById('deleteRoom')?.addEventListener('click',async()=>{if(!confirm('Excluir esta sala?')) return; const r=await api(`/admin/api/room/${roomCode}/delete`); if(r.ok) location.href='/admin/dashboard';});
  ...
  const saveNotes=()=>{
    ...
    notesTimer=setTimeout(()=>api(`/admin/api/room/${roomCode}/notes`,'POST',{summary_text:summaryText?.value||'',decisions_text:decisionsText?.value||''}),600);
  };
  summaryText?.addEventListener('input',saveNotes);
  decisionsText?.addEventListener('input',saveNotes);
}
...
function shouldInitiate(targetId){return Number(myId)>0&&Number(targetId)>0&&Number(myId)<Number(targetId);}
function createPeer(targetId){
  if(peers[targetId]) return peers[targetId];
  const pc=new RTCPeerConnection({iceServers:[{urls:'stun:stun.l.google.com:19302'},{urls:'stun:stun1.l.google.com:19302'}]});
  peers[targetId]=pc;
  if(localStream){localStream.getTracks().forEach(track=>pc.addTrack(track,localStream));}
  pc.onicecandidate=ev=>{if(ev.candidate) socket.emit('signal',{join_token:joinToken,target_id:targetId,type:'ice',candidate:ev.candidate});};
  pc.ontrack=ev=>{
    ...
    const selected=Number(currentState.room?.selected_id||currentState.room?.speaker_id||currentState.room?.screen_share_id||0);
    if(!selected||selected===Number(targetId)) promoteStream(stream,p?p.display_name:'Participante',false);
  };
  pc.onconnectionstatechange=()=>{if(['failed','closed','disconnected'].includes(pc.connectionState)) removeTile(targetId);};
  return pc;
}
async function callPeer(targetId){
  ...
  }catch(e){console.warn(e);}finally{pc._makingOffer=false;}
}
...
function syncPeers(){
  ...
  Object.keys(peers).forEach(id=>{if(!onlineIds.has(Number(id))) removeTile(Number(id));});
...
galleryBtn?.addEventListener('click',backToGallery);
